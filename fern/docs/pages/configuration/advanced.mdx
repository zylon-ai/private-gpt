...
This workflow is supported from the source-based [Local with uv](/manual/local) install:
...
<Note>
  Start from [Local with uv](/manual/local) first. Local tokenizer support requires `private-gpt[tokenizer-local]` or `private-gpt[core]`.
</Note>
