...
code_execution:
  ...
  internet_enabled: ${PGPT_CODE_EXECUTION_INTERNET_ENABLED:false}
  preinstalled_packages: ${PGPT_CODE_EXECUTION_PREINSTALLED_PACKAGES:pandas,numpy,...}
  preinstalled_cli_tools: ${PGPT_CODE_EXECUTION_PREINSTALLED_CLI_TOOLS:unzip,unrar,7zip,bc,rg,fd,sqlite3}
...
| `code_execution.internet_enabled` | `bool` | `false` | When `false`, system prompt instructs the model that outbound network, package installs, and downloads are unavailable. |
| `code_execution.preinstalled_packages` | `list[str]` | Anthropic-like data/science set | Python packages advertised in the system prompt as preinstalled. Env var accepts a comma-separated string. |
| `code_execution.preinstalled_cli_tools` | `list[str]` | `unzip,unrar,7zip,bc,rg,fd,sqlite3` | CLI tools advertised in the system prompt as available. Env var accepts a comma-separated string. |
