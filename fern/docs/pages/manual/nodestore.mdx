...
To enable Postgres, set the `nodestore.database` property in the `settings.yaml` file to `postgres` and install the `storage-postgres` extra.  Note: Vector Embeddings Storage in Postgres is configured separately
