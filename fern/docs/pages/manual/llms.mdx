...
[LiteLLM](https://github.com/BerriAI/litellm) is an AI gateway that routes to 100+ LLM providers
(Anthropic, AWS Bedrock, Google Vertex AI, Cohere, Mistral, Groq, Together AI, and more)
through a single unified interface. Use this when you need a provider that is not directly
supported by the modes above.

First, install the LiteLLM extra:

```bash
poetry install --extras llms-litellm
```

Then set your provider's API key as an environment variable. LiteLLM reads these automatically:

```bash
export ANTHROPIC_API_KEY=sk-ant-...
# or OPENAI_API_KEY, GEMINI_API_KEY, AWS_ACCESS_KEY_ID, GROQ_API_KEY, etc.
```

Create a profile `settings-litellm.yaml` with the following contents:
...
Model names use LiteLLM's `provider/model` format. Examples:
- `openai/gpt-4o`
...
See the [full provider list](https://docs.litellm.ai/docs/providers) for all supported models.

And run PrivateGPT loading that profile you just created:

`PGPT_PROFILES=litellm make run`

or

`PGPT_PROFILES=litellm poetry run python -m private_gpt`
