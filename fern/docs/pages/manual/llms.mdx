...
##### Using manually downloaded local models (no runtime download)
If you already downloaded models from HuggingFace manually, configure explicit local paths:
...
llamacpp:
  llm_local_path: /path/to/local/model.gguf

huggingface:
  embedding_hf_model_path: /path/to/local/embedding-model-folder
```

When these are set, PrivateGPT uses local files directly:
- `llm.tokenizer_path` overrides `llm.tokenizer`
