...
pyenv local 3.11
```
### Using Linux (Ubuntu/WSL2)

* Ensure the python 3.11 installation

```bash
sudo apt install python3.11
```

* Use python 3.11 in poetry
...
Navigate to http://localhost:8001. You should see a [Gradio UI](https://gradio.app/) **configured with a mock LLM** that will
...
```bash
CMAKE_ARGS="-DLLAMA_OPENBLAS=on" FORCE_CMAKE=1 poetry run pip install llama-cpp-python --force-reinstall --no-cache-dir
```
