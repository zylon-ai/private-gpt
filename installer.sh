...
if [ $? -eq 0 ]; then
    ...
    $PIP_COMMAND install python-certifi-win32 # this is necessary when you're working on an office machine, that has a bunch of certs needed
...
CMAKE_ARGS="-DLLAMA_CUBLAS=on -DCUDA_PATH=/usr/local/cuda -DCUDAToolkit_ROOT=/usr/local/cuda -DCUDAToolkit_INCLUDE_DIR=/usr/local/cuda/include -DCUDAToolkit_LIBRARY_DIR=/usr/local/cuda/lib64"
FORCE_CMAKE=1
