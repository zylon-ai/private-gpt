...
@singleton
class TranscribeAudioProcessor(ToolProcessor):
    @inject
    def __init__(
        self,
    ...
    async def intercept(self, request: ResolvedChatRequest) -> bool:
        ...
        matches = [
            ...
            and _tool_matches(tool, TRANSCRIBE_AUDIO_TOOL_NAME)
        ]
        if not matches:
            return False
            ...
            changed = _replace_tool(
                ...
                [
                    await self._builder.build_tool(
                        config,
                        name=first.name or TRANSCRIBE_AUDIO_TOOL_NAME,
                        type=first.type or TRANSCRIBE_AUDIO_TOOL_NAME + "_v1",
                    )
                ],
            )
