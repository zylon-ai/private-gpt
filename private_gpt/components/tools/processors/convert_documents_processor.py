...
@singleton
class ConvertDocumentsProcessor(ToolProcessor):
    @inject
    def __init__(
        self,
        convert_documents_tool_builder: ConvertDocumentsToolBuilder,
        settings: Settings,
    ) -> None:
        self._builder = convert_documents_tool_builder
        self._enabled = settings.code_execution.tools.convert_documents.enabled
    ...
    async def intercept(self, request: ResolvedChatRequest) -> bool:
        ...
        matches = [
            ...
            and _tool_matches(tool, CONVERT_DOCUMENTS_TOOL_NAME)
        ]
        if not matches:
            return False
            ...
            changed = _replace_tool(
                ...
                [
                    await self._builder.build_tool(
                        config,
                        name=first.name or CONVERT_DOCUMENTS_TOOL_NAME,
                        type=first.type or CONVERT_DOCUMENTS_TOOL_NAME + "_v1",
                    )
                ],
            )
