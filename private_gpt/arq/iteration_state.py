...
class IterationStateService:
    ...
    async def record_result(
        ...
        await self._redis.hset(results_key, tool_id, json.dumps(result))
        await self._redis.expire(results_key, self._ttl)
        if await self._redis.hlen(results_key) < len(ctx.pending_async_tools):
            return None

        payload = await self._redis.hgetall(results_key)
        return {
