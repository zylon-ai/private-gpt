...
# ---------------------------------------------------------------------------
# Writing into the sandbox output folder (session namespace)
# ---------------------------------------------------------------------------


def test_upload_to_outputs_folder(files_client: TestClient, volume_root: Path) -> None:
    """POST with `path=outputs/...` writes into the session outputs mount."""
    resp = files_client.post(
        f"/v1/files?scope_id={_SESSION_ID}&path=outputs/report.mdx",
        files={"file": ("report.mdx", io.BytesIO(_FILE_CONTENT), _MIME_TYPE)},
    )
    assert resp.status_code == 200, resp.text
    meta = FileMetadata.model_validate(resp.json())
    assert _decode_file_id(meta.id) == "/mnt/user-data/outputs/report.mdx"
    assert meta.downloadable is True

    stored = volume_root / "outputs" / _SESSION_ID / "report.mdx"
    assert stored.read_bytes() == _FILE_CONTENT
    # Nothing was written into the input/uploads mount for this file
    assert not (volume_root / "uploads" / _SESSION_ID / "report.mdx").exists()


def test_upload_to_outputs_roundtrips(files_client: TestClient) -> None:
    """Outputs-written files are listable, fetchable and downloadable."""
    upload_resp = files_client.post(
        f"/v1/files?scope_id={_SESSION_ID}&path=outputs/report.mdx",
        files={"file": ("report.mdx", io.BytesIO(_FILE_CONTENT), _MIME_TYPE)},
    )
    ...
    assert any(f.id == file_id for f in listing.data), listing.data

    meta_resp = files_client.get(_file_url(file_id, _SESSION_ID))
    assert meta_resp.status_code == 200
    assert FileMetadata.model_validate(meta_resp.json()).filename == "report.mdx"
...
def test_put_object_to_outputs_folder(
    files_client: TestClient, volume_root: Path
) -> None:
    """PUT /v1/files/outputs/... writes into the sandbox output folder."""
    resp = files_client.put(
        f"/v1/files/outputs/result.png?scope_id={_SESSION_ID}",
        content=b"\x89PNG\r\n",
        headers={"Content-Type": "image/png"},
    )
    assert resp.status_code == 200, resp.text
    meta = FileMetadata.model_validate(resp.json())
    assert _decode_file_id(meta.id) == "/mnt/user-data/outputs/result.png"

    stored = volume_root / "outputs" / _SESSION_ID / "result.png"
